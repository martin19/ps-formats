import {KnownEngineDataPropertyTypes} from "./EngineData";
...
function stringToPhotoshopUtf16ByteArray(str:string)
{
  ...
  {
    let charCode = str.charCodeAt(i);
    let d = [(charCode & 0xFF00) >>> 8, charCode & 0xFF];
...
function formatNumber(value:number, type:string) {
  ...
  if(typeof type === "string" && type === "float") {
    ...
    throw new Error("EngineDataWriter: unknonw number type.");
  }
  return str;
}
...
export class EngineDataWriter {
  ...
  static writeString(name : string|null, value:string, output : Array<number>, indent : number) {
    let data:Array<number> = [];
    if(name) {
  ...
  static writeArray(name : string, value:Array<any>, output : Array<number>, indent : number) {
    ...
    if(value.length && typeof value[0] === "number") {
      let rounded = value.map((f:number)=>{return formatNumber(f, KnownEngineDataPropertyTypes[name])});
      ...
      for(let i = 0; i < value.length;i++) {
        ...
        if(typeof v === "string") {
          ...
          throw new Error("unexpected array data type") + typeof v;
        }
      }
  ...
  static writeProperty(name : string|null, value:any, output : Array<number>, indent : number) {
    if(name) { 
      output.push.apply(output,ind(stringToArray("/" + name + "\n\t"),indent));
    }
    output.push.apply(output,ind(stringToArray("<<\n\t"),indent));
